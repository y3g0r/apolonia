...
namespace DiskUsageAnalyzer
{
	public partial class CircleMap : UserControl
	{
        ...
        public static float minAngle = 0.005f;
		...
		private void PaintFolderItem( FolderItem root, Graphics g)
		{
            ...
            if (root.SweepAngle < CircleMap.minAngle) // change to relation to the biggest segment in current circle
           // if (root.PercentageOfParent < 0.01) // this compare to the nearest paarent, not to root
			return;

            if (root.Level > FolderItem.maxLevel) return;
			foreach (FolderItem child in root.Children)
				PaintFolderItem(child, g);


			using (piepen = new Pen(pictureBoxMap.BackColor, 2f))
			{
